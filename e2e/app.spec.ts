...
test("category browsing, search, and copy workflow", async ({ context, isMobile, page }) => {
  ...
  await page.getByLabel("カテゴリ").getByRole("button", { name: /お気に入り/ }).click();
...
test("mobile keeps category, tag, search, and favorites surfaces available", async ({ context, isMobile, page }) => {
