...
現在の元データは `tmp/prompt-alchemy-assets` に仮置きされている。主な入力はカテゴリ付きタグの `tags.json` と、自由入力候補の `free-input-tags.json`。`tmp` は正式な管理場所ではなく、適切な時期に削除される前提で扱う。後続実装では、必要なデータをリポジトリ管理対象の正式なデータ入力元へ移し、恒久的なビルド、テスト、アプリ実行が `tmp` に依存しない状態にする。

`tags.json` は単一ファイルとしては大きいため、正式な入力データでは大カテゴリ単位に分割する。`free-input-tags.json` も単一ファイルとして扱わず、安定した分割規則で複数ファイルに分ける。データ形式は参照サイトとして高速に扱えるよう明確に定める。正式な入力データと、UI 表示・検索用に生成する派生データを分ける。派生データは手で編集するものではなく、入力データから再生成できる成果物として扱う。
...
`tmp/prompt-alchemy-assets` は初回移行時の一時的な入力元としてだけ扱う。恒久的なテスト、ビルド、アプリ実行、生成スクリプトの通常運用は `tmp` を参照してはならない。`tmp` との一致確認が必要な場合は、移行時だけ使う一時検証や手順メモに閉じ込め、通常の test script には含めない。
...
検索用インデックスは、日本語名、英語タグ、自由入力候補、カテゴリパスを対象にできる形にする。検索結果にはタグ名だけでなく、大カテゴリやサブカテゴリなどの文脈を表示し、結果から同じ分類内の近いタグを探し直せる導線を残す。生成インデックス内の各項目は、元の分割入力データへ戻れるカテゴリ ID、カテゴリパス、タグ ID または安定キーを持つ。
...
テスト方針は、データの分割と生成の正しさを中心に置く。Vitest を使い、正式入力データのスキーマ検証、分割データ全体としての復元性、変換ロジック、検索インデックス生成、カテゴリパスや参照キーの整合性を高速に検証する。Playwright は、実ブラウザでなければ意味が薄い主要導線に限定し、カテゴリ閲覧、検索、単発コピー、複数タグの一時選択とまとめてコピーが破綻しないことを確認する。
...
- `tmp/prompt-alchemy-assets` の仮置きデータから必要なデータが移行され、リポジトリ管理対象の正式な入力データとして `data/source/` 配下に整理されている。
- カテゴリ付きタグの正式入力データが、大カテゴリ単位の分割ファイルとして管理されている。
- 自由入力候補の正式入力データが、安定した規則の複数ファイルとして管理されている。
- 恒久的なビルド、テスト、アプリ実行が `tmp/` 配下のファイルに依存していない。
...
- Playwright で、カテゴリ閲覧、検索、単発コピー、一時選択リストからのまとめてコピーの主要導線が実ブラウザ上で確認されている。
